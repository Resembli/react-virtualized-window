...
Row height percent values are based on the `window` height. Colum width percent values are based on the the `window` width. Specify, for example,
a default value of `"10%"`, would mean we want 10 items to be visible.
...
Despite the explanation given above, you may notice that when you scroll quickly the contents within the window do not flashing in and out and the checkboarding
pattern is not present. This is due to a `sticky` positioning div that keeps the window in place and in view even as the user scrolls. For example, if we compare
...
from `react-virtualized-window`, we can see checkboarding vs no checkboarding in effect:
...
the checkboarding effect present in other `virtualization` frameworks, which is not present in `react-virtualized-window` (unless you disable the stick `div`).
