...
class TestTypeImpl {
  ...
  _createTest(type, location, title, fn) {
    ...
    const test = new _test.TestCase('test', title, fn, nextOrdinalInFile(suite._requireFile), this, location);
  ...
  _hook(name, location, fn) {
    ...
    if (name === 'beforeAll' || name === 'afterAll') {
      ...
      const hook = new _test.TestCase(name, name + suffix, fn, nextOrdinalInFile(suite._requireFile), this, location);
...
function nextOrdinalInFile(file) {
