...
var _blinkDiff = _interopRequireDefault(require("../third_party/blink-diff"));

var _pngJs = _interopRequireDefault(require("../third_party/png-js"));
...
function compareImages(actualBuffer, expectedBuffer, mimeType, options = {}) {
  ...
  if (process.env.PW_USE_BLINK_DIFF && mimeType === 'image/png') {
    const diff = new _blinkDiff.default({
      imageA: new _pngJs.default(expected),
      imageB: new _pngJs.default(actual)
    });
    const result = diff.runSync();
    return result.code !== _blinkDiff.default.RESULT_IDENTICAL ? {
      diff: PNG.sync.write(diff._imageOutput.getImage())
    } : null;
  }

  const count = (0, _pixelmatch.default)(expected.data, actual.data, diff.data, expected.width, expected.height, thresholdOptions);
  return count > 0 ? {
...
function compare(actual, pathSegments, testInfo, updateSnapshots, withNegateComparison, options) {
  ...
  if (!_fs.default.existsSync(snapshotFile)) {
    ...
    if (updateSnapshots === 'missing') {
      ...
      if (!('error' in testInfo)) testInfo.error = {
        value: 'Error: ' + message
      };else if ((_testInfo$error = testInfo.error) !== null && _testInfo$error !== void 0 && _testInfo$error.value) testInfo.error.value += '\nError: ' + message;
      return {
        pass: true,
        message
      };
