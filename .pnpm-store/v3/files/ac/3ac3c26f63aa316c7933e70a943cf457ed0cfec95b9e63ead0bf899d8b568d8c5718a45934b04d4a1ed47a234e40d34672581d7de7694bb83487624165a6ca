...
class RawReporter {
  ...
  generateProjectReport(config, suite) {
    ...
    for (const file of this.stepsInFile.keys()) {
      ...
      const highlighted = (0, _codeFrame.codeFrameColumns)(source, {
        start: {
          line: lines,
          column: 1
        }
      ...
      for (const step of this.stepsInFile.get(file)) {
        ...
        if (step.location.line < 2 || step.location.line >= lines) continue; // Cut out snippet.

        const snippetLines = highlightedLines.slice(step.location.line - 2, step.location.line + 1); // Relocate arrow.

        const index = lineWithArrow.indexOf('^');
        const shiftedArrow = lineWithArrow.slice(0, index) + ' '.repeat(step.location.column - 1) + lineWithArrow.slice(index); // Insert arrow line.
  ...
  _serializeSuite(suite) {
    ...
    const fileId = (0, _utils.calculateSha1)(location.file.split(_path.default.sep).join('/'));
    return {
      title: suite.title,
      fileId,
      location,
  ...
  _serializeResult(test, result) {
    return {
      ...
      error: (0, _base.formatResultFailure)(this.config, test, result, '', true).tokens.join('').trim(),
