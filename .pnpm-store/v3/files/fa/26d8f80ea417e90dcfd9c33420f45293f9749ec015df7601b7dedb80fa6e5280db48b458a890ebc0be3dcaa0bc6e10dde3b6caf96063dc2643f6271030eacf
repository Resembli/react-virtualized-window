...
class ListReporter extends _base.BaseReporter {
  ...
  _updateTestLine(test, line, suffix) {
    if (process.env.PWTEST_SKIP_TEST_OUTPUT) this._updateTestLineForTest(test, line, suffix);else this._updateTestLineForTTY(test, line, suffix);
  }
  ...
  _updateTestLineForTTY(test, line, suffix) {
    ...
    process.stdout.write(this._fitToScreen(line, suffix) + suffix); // Go down if needed.
  ...
  _fitToScreen(line, suffix) {
    const ttyWidth = this.ttyWidth();
    if (!ttyWidth) return line;
    return (0, _base.fitToScreen)(line, ttyWidth, suffix);
  }
