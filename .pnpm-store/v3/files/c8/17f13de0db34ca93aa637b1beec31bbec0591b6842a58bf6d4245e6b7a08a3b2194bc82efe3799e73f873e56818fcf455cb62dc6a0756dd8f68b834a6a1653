...
class Fixture {
  ...
  async setup(workerInfo, testInfo) {
    ...
    let called = false;
    const useFuncStarted = new _async.ManualPromise();
    ...
    const useFunc = async value => {
      ...
      called = true;
      this.value = value;
      this._useFuncFinished = new _async.ManualPromise();
      useFuncStarted.resolve();
      await this._useFuncFinished;
    };

    const info = this.registration.scope === 'worker' ? workerInfo : testInfo;
    this._selfTeardownComplete = (0, _util.wrapInPromise)(this.registration.fn(params, useFunc, info)).catch(e => {
...
class FixtureRunner {
  ...
  async resolveParametersForFunction(fn, workerInfo, testInfo) {
  ...
  async resolveParametersAndRunFunction(fn, workerInfo, testInfo) {
